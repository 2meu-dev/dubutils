exports.copystring = function (inputOptions) {
  ...
  const options = inputOptions.copystring();
  ...
  function get_PATH_TO_STRING() {
    ...
    throw new Error(
      `다음 파일[${searchThrough.join(
        ", "
      )}] 중 하나에 ${searchFor}를 정의하세요.
  ...
  async function theFunction() {
    ...
    if (copyEn) {
      if (
        !fs.existsSync(path.join(cleanPathToWorkingDir, "public/locales/en"))
      ) {
    ...
    if (copyKo) {
      if (
        !fs.existsSync(path.join(cleanPathToWorkingDir, "public/locales/ko"))
      ) {
    ...
    if (copyJa) {
      if (
        !fs.existsSync(path.join(cleanPathToWorkingDir, "public/locales/ja"))
      ) {
